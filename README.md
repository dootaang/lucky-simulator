...
전투/판정 시스템은 아카라이브 AI채팅 채널에 공유된 다음 모듈들의 메커니즘을 참고·편입하여 우리 엔진 방식으로 재구현했습니다(저작자 허락 하에, 조건: 사용 시 한 줄 크레딧).

- 🎲 Dice Module — 주사위 판정(굴림 vs 난이도 → 4단계 결과) 메커니즘. 저작: arca.live/b/characterai (공유글 링크는 배포 시 기입).
- ⚖️ Choice Module (Capsule Extension) — 성공확률 붙은 선택지 캡슐 UX. 저작: arca.live/b/characterai (공유글 링크는 배포 시 기입).

> 배포(아카라이브 업로드) 시점에 위 두 항목의 정확한 공유글 URL을 채워 넣을 것.
