...
describe('known card compiler',()=>{
  ...
  it('수치를 발명하지 않는다 — 카드 defaultVariables의 실능력치를 회수하고 없는 인형만 원본 폴백을 쓴다',()=>{
    ...
    expect(dolls[0]).toMatchObject({name:'D1',maxHp:1500,maxMp:1600,power:1250,mood:90}); // 카드 실값
  ...
  it('회수와 합성을 구분해 정직하게 표시한다(헌법 2)',()=>{
    ...
    expect(templateIssues.map(issue=>issue.path)).toEqual(expect.arrayContaining(['gfl.dolls','combat','gfl.manufacturing','gfl.facilities']));
