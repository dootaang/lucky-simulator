...
export function compileKnownCard(parsed:ParsedCard):CompileResult|null{
  ...
  const issues:CompileResult['issues']=[
    ...
    {level:'warn',path:'gfl.manufacturing',message:'제조 비용(인형 500/300 · 장비 300/200 · 중형 1500/1000/코어1)은 원본 흐름을 참고한 Lucky 규칙입니다 — 원본 수치와 다를 수 있습니다.',source:'template'},
    {level:'warn',path:'gfl.facilities',message:'시설 강화 비용의 레벨 배율은 Lucky 규칙입니다(기본 비용은 카드 base_defaults에서 회수).',source:'template'},
  ];
