...
function missionTheater(key:string){if(/^W/.test(key))return'east-europe';if(/^(?:S\d+A|TAURUSA|SATALA|S07A|HADLA)$/.test(key))return'east-china';return'red-orange';}
function missionRows(mined:ReturnType<typeof mineCard>){const east=table(mined.tables.EAST_FRONT_MISSIONS);return Object.entries(table(mined.tables.MISSION_DATA)).map(([key,value])=>{const row=record(value),isEast=Boolean(east[key]);return{id:id(key),code:key,name:text(row.name||key),theater:missionTheater(key),difficulty:text(row.diff),enemy:text(row.enemy),power:number(row.power,800),description:text(row.desc),rewards:reward(row.reward),rank:'E',east:isEast,travelCost:isEast?500:0};});}
...
function gflSchema(parsed:ParsedCard,mined:ReturnType<typeof mineCard>){const {rows:dolls,recovered}=dollRows(parsed,mined),items=itemRows(mined),equipment=equipmentRows(mined),missions=missionRows(mined),defaults=mined.defaultVars.numbers;return{recovered,schema:{
  meta:{id:'girls-frontline-ember',title:parsed.name,template:'genre.gfl',templateVersion:'1.2.0'},
  ...
  locations:[{id:'base-hall',name:'복도'},{id:'base-command',name:'지휘관실'},{id:'base-operation',name:'작전실'},{id:'base-maintenance',name:'정비실'},{id:'base-training',name:'훈련장'},{id:'base-warehouse',name:'물자창고'},{id:'base-dormitory',name:'인형숙소'},{id:'base-cafeteria',name:'카페테리아'},{id:'base-outside',name:'기지 외부'}].map(location=>({...location,links:location.id==='base-outside'?['base-hall']:['base-hall','base-command','base-operation','base-maintenance','base-training','base-warehouse','base-dormitory','base-cafeteria','base-outside'].filter(id=>id!==location.id)})),
