...
export interface LegacyLogEntry extends RuntimeRecord {
  ok: boolean;
  event: string;
}
export interface DispatchResult<TState extends RuntimeRecord = RuntimeRecord> {
  state: TState;
  log: LegacyLogEntry[];
}
export interface HandlerContext<TState extends RuntimeRecord = RuntimeRecord> {
...
export interface ModuleDefinition {
  ...
  stateAccess?: Partial<StateAccess>;
...
export interface NormalizedModule extends ModuleDefinition {
  dependencies: readonly string[];
...
export class ModuleRegistry {
  readonly #modules = new Map<string, NormalizedModule>();
  readonly #events = new Map<string, Route<EventHandler>>();
  readonly #selectors = new Map<string, Route<SelectorHandler>>();
  readonly #processes = new Map<string, Route<ProcessHandler>>();
  readonly #stateOwners = new Map<string, string>();
  register(definition: ModuleDefinition): this {
    const module = normalizeModule(definition);
    if (this.#modules.has(module.id))
      throw new Error(`duplicate_module:${module.id}`);
    for (const dependency of module.dependencies)
      if (!this.#modules.has(dependency))
        throw new Error(`missing_module_dependency:${module.id}:${dependency}`);
    this.#assertRoutes(this.#events, module.events, "event");
    this.#assertRoutes(this.#selectors, module.selectors, "selector");
    this.#assertRoutes(this.#processes, module.processes, "process");
    ...
    this.#registerRoutes(this.#events, module, module.events);
    this.#registerRoutes(this.#selectors, module, module.selectors);
    this.#registerRoutes(this.#processes, module, module.processes);
    for (const path of module.stateAccess.owns)
      this.#stateOwners.set(path, module.id);
    return this;
  }
  dispatch<TState extends RuntimeRecord>(
    schema: RuntimeRecord,
    state: TState,
    event: RuntimeEvent,
    rng: DeterministicRng,
  ): DispatchResult<TState> {
    const route = this.#events.get(event.id);
    if (!route) return unknownEvent(state, event.id);
    ...
    if (!validResult(result)) {
      ...
      return moduleFailure(
        ...
        event.id,
        "module_contract",
        "Module must return { state, log[] }.",
      );
    }
    if (!result.log.some((entry) => entry.ok)) {
      rng.restore(snapshot);
      return { state, log: result.log } as DispatchResult<TState>;
    }
    return result as DispatchResult<TState>;
  }
  select(id: string, ...args: unknown[]): unknown {
    const route = this.#selectors.get(id);
    if (!route) throw new Error(`unknown_selector:${id}`);
    return route.handler(...args);
  }
  runProcess(id: string, ...args: unknown[]): unknown {
  ...
  eventOwner(id: string): string | null {
    return this.#events.get(id)?.module.id ?? null;
  }
  selectorOwner(id: string): string | null {
    return this.#selectors.get(id)?.module.id ?? null;
  }
  stateOwner(path: string): string | null {
    return this.#stateOwners.get(path) ?? null;
  }
  getModule(id: string): NormalizedModule | null {
    return this.#modules.get(id) ?? null;
  }
  promptFacts(schema: RuntimeRecord, state: RuntimeRecord): RuntimeRecord {
    const out: RuntimeRecord = {};
    for (const module of this.#modules.values())
      if (module.promptFacts) {
        const value = module.promptFacts(schema, state);
        if (value !== undefined) out[module.id] = value;
      }
    return out;
  }
  ...
  listModules() {
    return [...this.#modules.values()].map((module) => ({
      ...
      stateAccess: {
        owns: [...module.stateAccess.owns],
        reads: [...module.stateAccess.reads],
        writes: [...module.stateAccess.writes],
      },
    }));
  }
  #assertRoutes<T>(
    ...
    for (const id of Object.keys(routes)) {
      ...
      if (existing)
        throw new Error(`duplicate_${kind}:${id}:${existing.module.id}`);
    }
  }
  #registerRoutes<T>(
    registry: Map<string, Route<T>>,
    ...
    for (const [id, handler] of Object.entries(routes))
      registry.set(id, { module, handler });
  }
}

function normalizeModule(definition: ModuleDefinition): NormalizedModule {
  if (!definition || typeof definition !== "object")
    throw new TypeError("invalid_module");
...
function normalizeStateAccess(access?: Partial<StateAccess>): StateAccess {
  return Object.freeze({
    owns: normalizePaths(access?.owns),
    reads: normalizePaths(access?.reads),
    writes: normalizePaths(access?.writes),
  });
}
...
function normalizeRoutes<T>(
  ...
  if (typeof routes !== "object" || Array.isArray(routes))
    throw new TypeError(`invalid_${kind}_routes:${moduleId}`);
  for (const [id, handler] of Object.entries(routes))
    if (!id.trim() || typeof handler !== "function")
      throw new TypeError(`invalid_${kind}_handler:${moduleId}:${id}`);
  return Object.freeze({ ...routes });
}
function validResult(result: unknown): result is DispatchResult {
  if (!result || typeof result !== "object") return false;
  const candidate = result as Partial<DispatchResult>;
  return (
    !!candidate.state &&
    ...
    log: [
      {
        ...
        detail: `Unknown event id: ${event}`,
      },
    ],
  } as DispatchResult<TState>;
}
function moduleFailure<TState extends RuntimeRecord>(
  state: TState,
  event: string,
  reason: string,
  detail?: string,
  ...
  return {
    state,
    log: [{ ok: false, event, reason, ...(detail ? { detail } : {}) }],
  } as DispatchResult<TState>;
}
