...
export const MODULE_CATALOG: Record<string, () => ModuleDefinition> = {
  ...
  'genre.gfl': gflModule,
  'sim.text-panels': textPanelsModule,
};
...
export function screenPresetsFor(moduleIds: string[], schema?:Record<string,unknown>): ScreenPresetBundle {
  ...
  if(moduleIds.includes('genre.gfl')){
    screens.push({id:'gfl-command',title:'그리폰 지휘부',layout:'gfl-command',regions:{main:[{widget:'gfl-console'}]}});
  }
  if(moduleIds.includes('sim.text-panels')){
    const declarations=(Array.isArray(schema?.textPanels)?schema.textPanels:[]) as TextPanelSchemaDecl[];
    if(declarations.length)screens.push({id:'text-panels',title:'상태창',layout:'dashboard',regions:{main:declarations.map(decl=>({widget:'detail-panel',title:decl.source||decl.id,source:`state.panels.${decl.id}${decl.kind==='panel'?'.fields':''}`}))}});
  }
  return {
    screens,
