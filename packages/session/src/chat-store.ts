...
export class ChatStore {
  ...
  async remove(chatId:string){const index=await this.list(),position=index.chats.findIndex((chat)=>chat.chatId===chatId);if(position<0)throw new Error(`chat_not_found:${chatId}`);index.chats.splice(position,1);if(index.activeChatId===chatId)index.activeChatId=index.chats[0]?.chatId??null;await this.#save(index);const sessionId=`${this.#projectId}:chat:${chatId}`,row=await this.#repository.get(sessionId),journal=(row?.payload as {journal?:{sealedEpochRefs?:Array<{offset:number}>}}|undefined)?.journal;for(const ref of journal?.sealedEpochRefs??[])await this.#repository.delete(`${sessionId}::sealed-epoch:${ref.offset}`); // 분리 보관된 봉인 본문 동반 삭제(파동 2 GC)
