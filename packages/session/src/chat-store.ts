...
export class ChatStore {
  ...
  for(let offset=0;offset<(payload?.shardManifest?.messages.chunks.length??0);offset+=1){await this.#repository.delete(`${sessionId}::shard:messages:${offset}:${payload!.shardManifest!.messages.chunks[offset]}`);await this.#repository.delete(`${sessionId}::shard:messages:${offset}`);}
  for(let offset=0;offset<(payload?.shardManifest?.journalEvents.chunks.length??0);offset+=1){await this.#repository.delete(`${sessionId}::shard:journal-events:${offset}:${payload!.shardManifest!.journalEvents.chunks[offset]}`);await this.#repository.delete(`${sessionId}::shard:journal-events:${offset}`);} // 샤드 청크 동반 삭제(파동 4 GC)
  await this.#repository.delete(`${sessionId}::action-wal`);
