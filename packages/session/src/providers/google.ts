...
export function createGoogleProvider(options:GoogleOptions):ModelProvider{
  ...
  return{async complete({prompt,signal,format='json'}){
    ...
    const text=(body.candidates?.[0]?.content?.parts??[]).map(x=>x.text??'').join('');if(!text)throw new Error('model_content_missing');return format==='prose'?{text,events:[],speakers:[],memories:[]}:jsonText(text);
  }};
}
export {promptParts as googlePromptParts};
