...
export interface CardRuntimeSnapshot{sessionId:string;cardId:string;revision:string;variables:Record<string,string>;randomState:number;logicalTimeMs:number}
...
export interface RuntimeClientWarning{code:'runtime_timeout'|'runtime_disposed'|'runtime_stale_response'|'runtime_worker_error'|'runtime_budget_rejected'|'runtime_soft_timeout'|'runtime_effect_blocked'|'runtime_effect_approval';requestId:string;message:string}
