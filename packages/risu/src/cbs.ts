...
function blocks(source:string,ctx:CbsContext){let text=source,guard=0;while(guard++<5000){const open=/\{\{#if(?:_pure)?(?:\s+|::)([^{}]*)}}/i.exec(text);if(!open)break;let depth=1,closeStart=-1,closeEnd=-1,elseStart=-1,elseEnd=-1;const scan=/\{\{(#if(?:_pure)?(?:\s+|::)[^{}]*|:else|\/(?:if)?)}}/gi;scan.lastIndex=open.index+open[0].length;let item:RegExpExecArray|null;while((item=scan.exec(text))){if(/^#if/i.test(item[1]!))depth++;else if(item[1]!.toLowerCase()===':else'&&depth===1){elseStart=item.index;elseEnd=item.index+item[0].length;}else if(--depth===0){closeStart=item.index;closeEnd=item.index+item[0].length;break;}}if(closeStart<0)break;const yes=text.slice(open.index+open[0].length,elseStart<0?closeStart:elseStart),no=elseStart<0?'':text.slice(elseEnd,closeStart),condition=inline(open[1]!,ctx);text=text.slice(0,open.index)+(truthy(condition)?yes:no)+text.slice(closeEnd);}return text;}
// Risu preset toggles commonly nest getglobalvar inside {{? ...}} inside #if.
// The upstream port loses the false branch when this exact nesting also has :else,
// so resolve only these read-only inputs first and let the small deterministic block
// evaluator choose the branch before handing all remaining CBS to the port.
function presetToggleBlocks(source:string,ctx:CbsContext){let text=String(source??''),guard=0;while(guard++<20){const next=text.replace(/\{\{\s*getglobalvar\s*::\s*([^{}]*?)}}/gi,(_whole,key:string)=>ctx.variables[key.trim()]??'').replace(/\{\{\s*\?\s*([^{}]*?)}}/g,(_whole,expression:string)=>calcString(expression));if(next===text)break;text=next;}return blocks(text,ctx);}
...
export function parseCbs(source:string,context:CbsContext):string{if((context.callStack??0)>=20)return'';
 ...
 setCbsPortEnv({
  ...
  const parsed=withCbsBudget(context.budget??new CbsBudget(),()=>risuChatParser(presetToggleBlocks(String(source??''),{...context,variables:store}),{chatID:context.chatIndex??-1,runVar:context.mutable===true,callStack:context.callStack??0}));
