...
export interface CompileCard{name:string;description?:string;personality?:string;scenario?:string;systemPrompt?:string;postHistoryInstructions?:string}
export interface PromptCompileInput{preset:PromptPreset;card:CompileCard;persona?:Persona|null;lore?:{entries:Array<{content:string;name?:string}>}|null;chat?:Array<{role:'user'|'assistant';content:string}>;authorNote?:{content:string;depth?:number}|null;memory?:string|null;engineContext?:{facts?:string;availableActions?:string;groundedMemory?:string}|null;variables?:Record<string,string|number|boolean>;options?:{mergeConsecutiveRoles?:boolean}}
...
export interface CompiledPrompt{messages:Array<{role:PromptRole;content:string}>;assistantPrefill:string;trace:PromptTraceRow[];warnings:Array<{code:string;path:string;detail?:string}>}
