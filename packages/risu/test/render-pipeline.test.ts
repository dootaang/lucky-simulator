...
describe('Risu 표시 파이프라인',()=>{
  ...
  it('CBS는 미지 함수를 잔재 없이 통과시키고 eval 없는 산술 결과를 낸다',()=>{expect(parseCbs('a{{unknown::x}}b',{variables:{}})).toBe('ab');expect(calcString('(2+3)*4')).toBe('20');expect(parseCbs('{{#when::{{greater::3::2}}}}yes{{:else}}no{{/when}}',{variables:{}})).toBe('yes');}); // ADR 0004: :else는 업스트림에서 #when 전용
  it('배경 CBS는 화면 너비를 계산하고 배경 에셋 매크로를 다음 단계까지 보존한다',()=>{const rendered=parseCbs('{{#when::{{? {{screen_width}} > 768 }}}}wide{{:else}}small{{/when}} {{bg::room}}',{variables:{},screenWidth:1200});expect(rendered).toBe('wide {{bg::room}}');});
});
