...
describe('card asset macros',()=>{
  ...
  it('expands Risu asset lists from the real card and module catalog',()=>{const preset=defaultCardPreset();preset.blocks=[{id:'main',type:'plain',name:'main',enabled:true,role:'system',text:'E={{emotionlist}} A={{assetlist}} M={{moduleassetlist::gfl}}',source:null}];const result=compilePrompt({preset,card:{name:'Card'},assets:[{name:'face_smile',type:'emotion',mime:'image/png',bytes:null},{name:'room',type:'image',mime:'image/png',bytes:null},{name:'HK416_default',type:'module-asset',mime:'image/png',bytes:null,moduleNamespace:'gfl'}]});expect(result.messages[0]?.content).toBe('E=["face_smile"] A=["room"] M=["HK416_default"]');expect(result.warnings).toEqual([]);});
