...
// 플레이어·UI 컴포넌트의 직접 색상 리터럴 수를 기준 목록(baseline)과 비교한다.
...
const COLOR_LITERAL = /#[0-9a-fA-F]{3,8}\b|(?:rgb|hsl)a?\([^)]*\)/g;
const UI_EXTENSIONS = ['.svelte', '.css'];

function uiFiles(dir) {
  return readdirSync(dir, { withFileTypes: true, recursive: true })
    .filter((entry) => entry.isFile() && UI_EXTENSIONS.some((extension) => entry.name.endsWith(extension)))
    .map((entry) => join(entry.parentPath ?? entry.path, entry.name).replaceAll('\\', '/'));
}

const counts = {};
for (const root of ROOTS) for (const file of uiFiles(root)) {
  const found = readFileSync(file, 'utf8').match(COLOR_LITERAL)?.length ?? 0;
...
if (offenders.length) {
  console.error('직접 색상 리터럴이 기준보다 늘었습니다. 디자인 토큰(var(--color-*))을 사용하세요.');
