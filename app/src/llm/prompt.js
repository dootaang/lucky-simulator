...
- upgrade {facility} — 시설 확장. facility는 상태의 시설 id(tavern/kitchen/room/quarters). (엔진: 비용 차감 + 레벨 상승. 비용·골드·레벨은 쓰지 마라)
- gain_resource {resource:"food"|"drink", scale:"small"|"large"|"bulk", reason} — 사냥·채집·부산물로 재료를 얻을 때. (엔진: 규모 표에서 수량 결정. qty·amount·gold는 쓰지 마라)
...
- exp_gain {category, reason} — 의미 있는 행동에만, 매 턴 금지. category는 [경험치 카테고리] 목록에서만(목록에 없으면 내지 마라). (엔진이 값 결정)
