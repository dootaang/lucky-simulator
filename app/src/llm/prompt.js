...
- checkin {roomNo, guestName, stayDays} / checkout {roomNo, guestName} — roomNo는 [객실 목록]의 실제 호수(예: 101)를 쓴다
...
- ★사건을 부풀리지 마라. 유저가 지시했거나 장면에서 실제로 완결된 거래 1건당 사건 1개만. 손님 수·주문량을 임의로 늘려 sale을 여러 개 만들지 마라. 유저가 "하나 팔자"면 sale 1개, "더 팔자"처럼 모호하면 장면에 실제 등장해 완결된 판매만 센다.
- 판매는 재료를 소비하고 팔린다(엔진이 식자재/주류를 차감). 재고 없이 무한 판매는 없다.
- npcId는 [NPC 목록]의 영문 id만 사용.
- sale의 menuName은 [메뉴 목록]에 있는 이름을, checkin의 roomNo는 [객실 목록]의 호수를 정확히 그대로 사용한다(목록에 없으면 불가).
