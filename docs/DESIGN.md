...
관리 요약은 장르별 새 화면을 복제하지 않는다. 각 모듈이 이미 제공하는 세 파생물을 공용 셸이 재배치한다.

1. `지금 상태`: 스키마 `hud` 선언 또는 표준 상태에서 만든 `HudModel`
2. `지금 할 일`: selector가 현재 가능하다고 판정한 `DecisionCardModel`
3. `최근 결과`: 엔진이 확정한 최근 `lastLogs` 영수증

공용 셸은 위 세 자료의 개수만 제한하며 장르 ID를 보고 분기하지 않는다. 빠른 관리 시트와 관리 탭은 같은
`ManagementSummaryModel`을 사용하고, 전체 기능만 선언형 `ScreenRenderer`/네이티브 콘솔로 넘긴다. 따라서
후속 장르는 계기판·결정 selector·엔진 로그를 제공하면 요약층에 자동으로 들어오며, 별도 앱 UI 복사는 금지한다.
