...
## 2026-07-23 세션 메모 — 모듈화·관리 요약·문서 정합

- [x] `PlayerPage`의 AI 서비스 설정 읽기·전환·보조 모델 구성을 `player-settings.ts`로 분리
- [x] `genre.gfl`의 병과 전투 프로필·진형·지휘관 레벨·세력 상성을 `gfl-tactics.ts`로 분리
- [x] 첫 진입 메뉴 이동 안내와 설정의 다시 보기, 과거 대화 스크롤 위치 보존 회귀 추가
- [x] `봇·대화·관리·설정` 네 목적지와 장르 중립 관리 요약 계약 구현
- [x] 빠른 관리 시트와 관리 탭이 같은 계기판·결정 카드·최근 영수증을 사용하도록 연결
- [x] Alternate Hunters V2를 플랫폼 준비 4단계(두 번째 대형 선언형 어댑터)의 완료 증거로 고정
- [ ] 파동 7 비코더 실기기 사용자 시험과 GFL 계층 2 오너 관문
- [ ] 다음 카나리아 DOMINIUM: 컴파일러 선언형화 관통
