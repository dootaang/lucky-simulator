...
1. **오너 수동 실물 스모크**: 용사여관을 실제 브라우저·모바일에서 굴려 감정 스프라이트, 관리 행동 → 채팅,
   저장 후 새로고침, 진단 콘솔 복사 내용을 확인한다. 자동 테스트 통과를 실물 합격으로 대신하지 않는다.
2. **테스트 전용 런타임 카나리아**: 용사여관·DOMINIUM·Tesselia의 실제 트리거/Lua를 제품 배선 없이 Worker에
   넣어 차단 효과, 필요한 변수 API, 실행 시간·메모리를 측정한다. 정상 카드를 맞추려고 상한부터 올리지 않는다.
...
5. **기본 플레이 공백 복구**: 전투 콘솔+소모품 → 여관 상거래·채집·숙박 일괄 → 전투·아이템 결과 영수증 순서.
   `docs/PLAY-PARITY-AUDIT.md`의 “엔진 사건 존재”를 사용자 기능 완료로 오인하지 않는다.
6. **두 번째 무코드 카나리아**: 헌터 등록·평가·파티·게이트·전투·정산 화면과 DOMINIUM을 카드명 전용 앱 코드 없이
   선언형 화면·장르 모듈만으로 완주한다.
