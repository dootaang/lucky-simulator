...
- [ ] `SessionStore` 계약: session, message, engine event, snapshot, memory, prompt run, asset reference
- [ ] append-only 엔진 이벤트 로그 + 주기적 snapshot + schema migration
- [ ] 모든 LLM 요청에 prompt hash, 사용 블록, 모델, 응답, 제안 사건, 적용/거부 결과 기록
- [ ] 턴·날짜·사건 단위 체크포인트와 time machine 복원
- [ ] 브라우저 SQLite WASM + OPFS Worker 저장 어댑터
- [ ] Tauri 데스크톱 네이티브 SQLite 저장 어댑터
- [ ] 내보내기 가능한 단일 세션 백업과 손상 복구 검사
...
- [ ] 300턴 결정론 자동 플레이 fixture
- [ ] 저장→종료→재개 후 state hash·RNG·선택지 일치
