...
- [ ] 채팅별 `boundPersonaId`와 시작 시점 persona snapshot 저장 — 현재는 카드별 선택만 저장하며 채팅/세션 snapshot에는 바인딩이 없다
- [ ] 플레이 도중 페르소나를 수정해도 과거 세션의 사실이 소급 변경되지 않도록 버전·snapshot 분리 — 라이브러리·카드별 선택은 있으나 `SessionSnapshot`에 세션 시작 시점 페르소나가 고정되지 않는다 (`MIGRATION-PARITY-AUDIT`)
- [ ] `{{user}}`, 사용자 이름, 페르소나 설명, 아이콘을 프롬프트·세션·내보내기에 동일하게 연결 — 현재 프롬프트 연결은 되지만 세션 백업에는 페르소나 snapshot이 없다
...
- [ ] 페르소나 라이브러리와 세션 바인딩 — 라이브러리·카드별 선택은 구현, 세션 시작 시점 snapshot 고정은 유실
...
- [ ] `SessionStore` 계약: 현재 상태 저장소는 구현됐지만 message/engine event/snapshot/prompt run의 append-only 저장 계약은 `03074d8`에서 유실. `packages/contracts/src/session.ts`의 `SessionBundle`은 사용처 없는 계약만 남음
- [ ] append-only 엔진 이벤트 로그 + 주기적 snapshot — 현재 상태·RNG snapshot과 무결성 해시는 대체됐지만 사건열·중간 snapshot·재생은 유실
- [ ] 모든 LLM 요청에 prompt hash, 모델, 응답, 제안 사건, 적용 수 기록 — 채팅 LLM의 상세 `PromptRun`은 기록하지만 최근 8개만 저장하며, 유실된 현장·일괄·장부 서사화 경로는 기록되지 않음
- [ ] 사건 index 단위 체크포인트(`stateAt`)와 time machine 복원(`truncateTo`) — 같은 탭의 최근 30턴 undo/redo만 있고 새로고침 뒤 이력과 사건 index 복원은 유실
- [x] 브라우저 SQLite WASM + OPFS Worker 저장 어댑터 (`@sqlite.org/sqlite-wasm` 직접 Worker + IndexedDB 안전 폴백)
- [ ] Tauri 데스크톱 네이티브 SQLite 저장 어댑터
- [ ] 내보내기 가능한 단일 세션 백업과 손상 복구 검사 — 현재 상태·대화·기억·대안·최근 8개 PromptRun은 무결성 해시로 왕복하지만 사건 원장·영속 undo/redo·세션 고정 페르소나/프리셋·가져오기 크기 상한은 유실

### 기억 계층

- [x] 엔진 현재 사실: 돈·HP·관계·퀘스트·위치 등 절대 요약하지 않는 authoritative state
- [ ] 사건 원장: 누가 언제 무엇을 했고 어떤 state diff가 났는지 구조화 저장 — 현재는 마지막 실행 로그와 현재 상태만 저장
...
- [ ] 300턴 사건 원장 재생 fixture — 현행 300턴 테스트는 현재 상태·RNG·기억 저장/복원과 프롬프트 상한을 검증한다. 300개 사건의 append-only 재생·중간 시점 복원·분기·손상 검사는 유실
...
### 현재 구현 감사 — 현재 상태 저장은 대체, 사건 원장·영속 이력은 유실 (2026-07-14 재감사)

- [ ] 300개 **엔진 사건**의 원장·스냅샷·재생·분기·손상 감지 — 삭제된 `engine/test/sessionJournal.test.js`를 가리키던 거짓 완료. 현재 상태 300턴 복원만 별도로 통과
- [ ] 플레이 화면의 수동 세션 JSON 내보내기/가져오기와 이어하기 — 현재 snapshot 백업은 가능하나 사건 원장·영속 undo/redo·세션 고정 페르소나/프리셋은 포함하지 않음
...
- [ ] PromptRun 전체 계약 보존: 현재 플레이 snapshot은 상세 실행 자료를 최근 **8건**만 유지한다 (옛 구현은 최근 500건)
