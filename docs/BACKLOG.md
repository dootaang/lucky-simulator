...
- [ ] **C0 벤치마크 보정**: 관련 없음(abstention), 최근 3개 메시지 질의, current fact exact match, NPC 혼동, 비밀 지식 범위, 롤백 기억, 근거 보유율/정답 근거 적중률 분리
- [ ] **C1 선 구현**: `VoyageEmbeddingProvider`를 TypeScript strict로 작성하고 mock fetch로 배치·캐시·429 backoff·취소·키 마스킹·장애 폴백 검증
- [ ] **C2 비교 기준 E — Grounded Hybrid V2**: authoritative facts + typed memory + lexical prefilter + 선택형 semantic + evidence/knowledge/rollback gate + abstention
