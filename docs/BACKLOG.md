...
- [ ] 300턴 대화형 고정 corpus와 질문 정답지 작성: 현재 사실, 최신값 우선, 약속·비밀, 의미 바꿔 말하기, NPC 구분, 무관 기억 배제
- [ ] 비교 기준 A: 최근 대화 창만 사용
- [ ] 비교 기준 B: 구조화 사실 + 키워드/FTS 검색
- [ ] 비교 기준 C: Risu HypaMemory V3 방식(중요·최근·유사 기억, 최근 3개 질문, 요약 조각 임베딩)
- [ ] 비교 기준 D: 우리 hybrid(엔진 사실 + 사건 원장 + FTS + 선택형 semantic retrieval)
- [ ] Voyage `voyage-context-3`를 Risu 재현 기준으로 측정하고 `voyage-context-4`를 최신 대안으로 별도 측정
- [ ] Recall@K·MRR/nDCG·현재 사실 정확도·폐기된 과거값 오답률·근거 정확도·토큰·p50/p95 지연·API 토큰/비용 기록
- [ ] 외부 API 없는 고정 임베딩 provider로 CI 결정론 유지; 실 Voyage 벤치마크는 BYOK opt-in 별도 명령
- [ ] 결과 보고서(JSON + Markdown) 전에는 플레이 경로에 임베딩 검색을 연결하지 않음
- [ ] RisuAI GPL 코드 선별 이식 시 repo·commit·원본 파일·함수·수정 내용을 provenance 문서에 기록
...
5. Voyage의 이득이 FTS 대비 작으면 외부 임베딩을 기본 기능으로 만들지 않을 것
