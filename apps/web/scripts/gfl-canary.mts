...
const nativeSchema=compiled.schema.gfl as{dolls:Array<Record<string,unknown>>;missions:Array<Record<string,unknown>>;items:Array<Record<string,unknown>>;progression:{byStar:Record<string,number>;missionTypes:Array<Record<string,unknown>>;eventGuides:Record<string,string>};encounters:{pool:string[];ban:string[]}};
...
console.log(JSON.stringify({
  ...
  native:{modules:compiled.moduleIds,dolls:nativeSchema.dolls.length,missions:nativeSchema.missions.length,theaters:theaters.map(value=>value.name),echelons:echelons.length,starter:null,hired:owned[0]!.name,hireOffers:hire.offers.length,dormCapacity:hire.capacity,m4ContractPrice:m4?.price,facilityCosts:[firstUpgrade.log[0]?.cost,secondUpgrade.log[0]?.cost],progression:{byStar:nativeSchema.progression.byStar,missionTypes:nativeSchema.progression.missionTypes.length,eventGuides:Object.keys(nativeSchema.progression.eventGuides).length,itemDrops:nativeSchema.items.filter(item=>typeof item.drop==='number').length,encounterPool:nativeSchema.encounters.pool.length,encounterBan:nativeSchema.encounters.ban.length}},
  assetModules,
