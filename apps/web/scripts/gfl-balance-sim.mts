...
function schema(slots:string[],ratio:number){const required=missionPower/ratio,enemies=Array.from({length:5},(_,i)=>({id:`enemy${i}`,power:required/5,hp:required/5*1.5}));return{initialState:{day:1,gold:0,resources:{},items:{},player:{pools:{hp:{cur:1,max:1},mp:{cur:1,max:1}}},clock:{day:1,hour:8,turn:0},location:"base-command",gfl:{started:true,dolls:Object.fromEntries(dolls.map(unit=>[unit.id,{...unit,hp:{cur:unit.maxHp,max:unit.maxHp},basePower:unit.power,power:unit.power,status:"대기",equipment:[]}])) ,echelons:[{id:"e1",slots}],facilities:{},completedMissions:[],sortie:null,daily:{day:1,sortiesUsed:0,sortiesCompleted:0}}},resources:[],gather:{},party:{maxSize:6,roles:[]},time:{hoursPerStep:4},jobs:[],locations:[],combat:{},skills:{},gfl:{dolls,items:[],equipment:[],fairies:[],missions:[{id:"sim",name:"SIM",power:required,enemy:"철혈",factions:["철혈"],enemies,rewards:{}}],facilities:[],hire:{},manufacturing:{}}};}
function win(slots:string[],ratio:number,seed:number){const source:any=schema(slots,ratio), registry=createCoreRegistry().register(gflModule()), rng=createRng(seed);let state=createState(source,seed);const start=registry.dispatch(source,state,{id:"gfl/sortie/start",params:{missionId:"sim",echelonId:"e1"}},rng);state=start.state;return (registry.dispatch(source,state,{id:"gfl/sortie/resolve",params:{tactic:"balanced"}},rng).log[0] as any).outcome==="victory";}
console.log(`GFL combat composition Monte Carlo — seeds 1..${runs}`);
console.log("| 편성 | 전투력비 0.5 | 0.75 | 1.0 | 1.5 |"); console.log("|---|---:|---:|---:|---:|");
const results:Record<string,number[]>={};
for(const [name,slots] of Object.entries(formations)){results[name]=ratios.map(ratio=>Array.from({length:runs},(_,i)=>win(slots,ratio,i+1)).filter(Boolean).length/runs*100);console.log(`| ${name} | ${results[name]!.map(value=>`${value.toFixed(1)}%`).join(" | ")} |`);}
const proper=results["균형 정배치"]![2]!, reverse=results["균형 역배치"]![2]!, low=Math.max(...Object.values(results).map(row=>row[0]!));
const passed=proper>=55&&proper<=85&&proper-reverse>=5&&proper-reverse<=20&&low<30;
console.log(`criteria: ratio1 proper=${proper.toFixed(1)}%, placement delta=${(proper-reverse).toFixed(1)}%p, ratio0.5 max=${low.toFixed(1)}% => ${passed?"PASS":"FAIL"}`);
