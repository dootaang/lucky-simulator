...
export class AssetLoadScheduler {
  ...
  schedule<T>(task:()=>Promise<T>,signal?:AbortSignal):Promise<T>{return new Promise<T>((resolve,reject)=>{const start=()=>{if(signal?.aborted){reject(signal.reason??new DOMException('Aborted','AbortError'));this.#pump();return;}this.#active+=1;this.#started+=1;this.#peak=Math.max(this.#peak,this.#active);void task().then(value=>{this.#completed+=1;resolve(value);},reason=>{this.#failed+=1;reject(reason);}).finally(()=>{this.#active-=1;this.#pump();});};this.#queue.push(start);this.#pump();});}
