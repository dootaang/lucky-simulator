<script lang="ts">
  ...
  let { profile, passport, index, session, version, bindingUi, assets, assetUrlFor, open = false, oncreate, onselect, onrename, onremove, onexport, onimport, onundo, onredo }: {
    profile: CardRuntimeProfile; passport: CardPassport; index: ChatIndex; session: PlaySession; version: number;
    bindingUi: BindingUi; assets: CardAsset[]; assetUrlFor: (asset: CardAsset) => string | null; open?: boolean;
    oncreate: () => void; onselect: (id: string) => void; onrename: (id: string) => void; onremove: (id: string) => void;
    onexport: () => void; onimport: (file: File) => void; onundo: () => void; onredo: () => void;
  } = $props();
...
<aside class="side" class:open style:display={open?'flex':undefined} style:background="var(--card-atmosphere-bg,#0f1013)" style:border-color="var(--card-atmosphere-border,#2c2f36)">
  ...
  {#if tab==='chat'}
    ...
    <section class="engine"><div class="engine-head"><strong><Icon name="star" size={12}/> 엔진 확정 · 이번 턴</strong><span>{confirmed}확정 · {blocked}차단</span></div><div class="mode">{passport.mode==='full-sim'?'완전 시뮬':'일반 채팅'}</div>{#if stateRows.length}<dl>{#each stateRows as row}<div><dt>{row.label}</dt><dd>{row.value}</dd></div>{/each}</dl>{/if}{#if facts.length}<ul>{#each facts.slice(0,4) as fact}<li class:rejected={fact.rejected}><Icon name={fact.icon} size={13}/><b>{fact.label}</b><span>{fact.delta??fact.note}</span></li>{/each}</ul>{/if}<div class="history"><button disabled={!session.checkpointDepth} onclick={onundo} title="되돌리기"><Icon name="undo"/><span>{session.checkpointDepth}</span></button><button disabled={!session.redoDepth} onclick={onredo} title="다시 실행"><Icon name="redo"/><span>{session.redoDepth}</span></button></div></section>
  {:else if tab==='basic'}
