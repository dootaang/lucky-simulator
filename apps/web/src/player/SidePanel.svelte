<script lang="ts">
  ...
  let { profile, passport, index, session, version, bindingUi, assets, assetUrlFor, open = false, compiling = false, oncreate, onselect, onrename, onremove, onexport, onimport, oncompile, oninspect, onundo, onredo }: {
    ...
    onexport: () => void; onimport: (file: File) => void; oncompile: () => void; oninspect: () => void; onundo: () => void; onredo: () => void;
  } = $props();
