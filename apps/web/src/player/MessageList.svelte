<script lang="ts">
  ...
  let menuFor=$state<string|null>(null); // ⋯ 팝업이 열린 메시지
  let messages=$derived.by(()=>{void version;void live;return session.messages;});
  let facts=$derived.by(()=>{void version;void live;return session.lastLogs.map(toFactLine);});
  ...
  // 리스 문법: 삭제는 항상 확인을 묻는다. 이후 전부 삭제는 ⋯ 메뉴 안에 있고 별도로 다시 묻는다.
