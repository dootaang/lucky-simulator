...
export function buildNpcClusters(assets:readonly CardAsset[]):NpcCluster[]{
  const seen=new Set<string>(),pictures=assets.filter(image).filter(asset=>{const value=key(stem(asset));if(seen.has(value))return false;seen.add(value);return true;}),stems=pictures.map(stem),bareOwners=stems.filter(value=>stems.some(other=>other!==value&&(other.startsWith(`${value}_`)||other.startsWith(`${value} `)||other.startsWith(`${value}.`)))),owners=[...new Set([...pictures.map(asset=>defaultOwner(stem(asset))).filter((value):value is string=>!!value),...bareOwners])],groups=new Map<string,NpcSprite[]>();
