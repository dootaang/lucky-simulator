...
describe('safe Risu background rendering',()=>{
  ...
  it('keeps card assets ahead of modules and permits an explicit module namespace',()=>{const card={...asset},module={...asset,moduleNamespace:'pack'};expect(safeBackgroundSource('{{raw::room.png}} {{raw::pack/room.png}}',[card,module],value=>value===card?'blob:card':'blob:module')).toContain('blob:card blob:module');});
});
