...
export function traceRun(run:Run,card:string,chat:string,message:number|null){
  ...
  const kind=KIND[run.kind]??run.kind;
  ...
  if(run.prompt){
    ...
    diagnostics.record({...where,level:'info',kind:'provider',code:'prompt_built',key:`run:${run.id}:prompt`,summary:`프롬프트 조립 · ${kind}`,
  ...
  diagnostics.record({...where,level:'info',kind:'provider',code:'model_response',key:`run:${run.id}:model`,summary:`모델 응답 · ${run.model??'(모델 미상)'} · ${ms(run.durationMs)}`,
  ...
  if(run.finishReason==='length')diagnostics.record({...where,level:'warn',kind:'provider',code:'response_truncated',key:`run:${run.id}:truncated`,summary:'응답이 토큰 상한에서 잘렸다',detail:{'종료 사유':'length','조치':'설정에서 최대 토큰을 올리거나 프롬프트를 줄인다'}});

  // ③ 엔진 사실 — 모델이 제안한 이벤트 중 무엇이 적용되고 무엇이 차단됐는가. 차단은 조용히 넘어가지 않는다.
  const facts=run.logs.map(toFactLine),rejected=facts.filter(fact=>fact.rejected);
  if(run.proposedEvents.length||facts.length)diagnostics.record({...where,level:'info',kind:'simulation',code:'engine_facts',key:`run:${run.id}:engine`,summary:`엔진 사실 ${facts.length-rejected.length}건 적용${rejected.length?` · ${rejected.length}건 차단`:''}`,
  ...
  for(const fact of rejected)diagnostics.record({...where,level:'warn',kind:'simulation',code:'engine_event_blocked',key:`run:${run.id}:blocked:${fact.key}`,summary:`엔진이 차단: ${fact.label}`,detail:{'이유':fact.note||'(없음)','원칙':'LLM은 사실을 직접 바꾸지 못한다 — 서술만 한다'}});

  // ④ 기억 — 근거 없는 기억은 저장되지 않는다. 왜 안 남았는지가 보여야 한다.
  const decisions=run.memoryDecisions??[],approved=decisions.filter(value=>value.status==='approved').length,pending=decisions.filter(value=>value.status==='candidate').length;
  if(decisions.length)diagnostics.record({...where,level:'info',kind:'card',code:'memory_decided',key:`run:${run.id}:memory`,summary:`기억 ${approved}건 확정 · ${pending}건 검토 대기 · ${decisions.length-approved-pending}건 폐기`,
    detail:{'결정':decisions.map(value=>`${value.status}(${value.reason})`).join(', ')}});

  // 서사 검증 문제(엔진 사실과 어긋나는 서술 등)는 경고다.
  for(const issue of run.issues)diagnostics.record({...where,level:'warn',kind:'card',code:`narrative_${issue.code}`,key:`run:${run.id}:issue:${issue.code}`,summary:`서사 검증: ${issue.code}`,detail:{'상세':issue.detail??'(없음)'}});
}
...
export function createTurnTracer(){
  ...
  return(session:PlaySession|null,card:string)=>{
    if(!session||!diagnostics.enabled('info'))return;
    const runs=session.promptRuns;
    if(runs.length<seen)seen=0; // 채팅을 바꾸면 다시 센다
    for(let index=seen;index<runs.length;index+=1)try{traceRun(runs[index]!,card,session.id,session.messages.length-1);}catch{/* 진단이 플레이를 해치지 않는다 */}
    seen=runs.length;
  };
}
