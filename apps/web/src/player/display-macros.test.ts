...
describe('lazy Risu asset display',()=>{it('never exposes an unresolved img CBS and paints it after its module URL is ready',()=>{const asset={name:'FAMAS_smile',type:'module-asset',mime:'image/png',bytes:null as Uint8Array|null};let url:string|undefined;const options={urlFor:()=>url},pending=renderDisplayContent('앞 {{img::FAMAS_smile}} 뒤','User','Card',[asset],[],{},0,0,options);expect(pending.html).toContain('앞  뒤');expect(pending.html).not.toContain('{{img::');expect(pending.warnings).toContainEqual({code:'asset_missing',macro:'img',name:'FAMAS_smile'});url='blob:https://local.invalid/famas';const ready=renderDisplayContent('앞 {{img::FAMAS_smile}} 뒤','User','Card',[asset],[],{},0,0,options);expect(ready.html).toContain('<img src="blob:https://local.invalid/famas" alt="FAMAS_smile">');expect(ready.warnings).toEqual([]);});});
