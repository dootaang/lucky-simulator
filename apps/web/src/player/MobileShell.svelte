<script lang="ts">
  ...
  let{cardName='봇을 선택하세요',cards,index,activeId,hasCard=false,sideOpen=false,onadd,oncard,onremove,onsideopen=()=>{},onsideclose=()=>{},onsettings}:{cardName?:string;cards:CardLibraryMeta[];index:ChatIndex;activeId:string|null;hasCard?:boolean;sideOpen?:boolean;onadd:()=>void;oncard:(id:string)=>void;onremove:(id:string)=>void;onsideopen?:()=>void;onsideclose?:()=>void;onsettings:(tab:MobileSettingsTab)=>void}=$props();
