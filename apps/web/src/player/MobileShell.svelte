<script lang="ts">
  ...
  let{cardName='봇을 선택하세요',cards,index,activeId,hasCard=false,sideOpen=false,onadd,oncard,onsideopen=()=>{},onsideclose=()=>{},onsettings}:{cardName?:string;cards:CardLibraryMeta[];index:ChatIndex;activeId:string|null;hasCard?:boolean;sideOpen?:boolean;onadd:()=>void;oncard:(id:string)=>void;onsideopen?:()=>void;onsideclose?:()=>void;onsettings:(tab:MobileSettingsTab)=>void}=$props();
  let page=$state<'chat'|'library'>('chat'),query=$state('');
  let filtered=$derived(cards.filter(card=>card.name.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase())));
  $effect(()=>{if(!hasCard)page='library';});
  function choose(id:string){page='chat';oncard(id);}
  function settings(){onsideclose();onsettings('model');}
</script>
...
{#if page==='library'&&!sideOpen}
  <section class="library" aria-label="봇 목록">
    ...
    <div class="cards">
      {#each filtered as card(card.projectId)}<button class:active={card.projectId===activeId} onclick={()=>choose(card.projectId)}><span class="avatar">{#if card.thumbnail}<img src={card.thumbnail} alt=""/>{:else}{card.name.slice(0,1)}{/if}</span><span class="meta"><b>{card.name}</b><small>{card.projectId===activeId?`${index.chats.length}개 채팅 · 현재 사용 중`:card.missing?'원본 재연결 필요':'열어서 계속하기'}</small></span><Icon name="right" size={18}/></button>{/each}
...
<style>
  ...
  @media(max-width:999px){
    ...
    .library{display:flex;position:fixed;inset:calc(56px + env(safe-area-inset-top)) 0 0;z-index:30;flex-direction:column;background:#111318;color:#eef0f5}.search{display:flex;align-items:center;gap:9px;margin:12px;padding:0 12px;border:1px solid #363b46;border-radius:9px;background:#181b21;color:#858d9c}.search input{flex:1;min-width:0;padding:12px 0;border:0;outline:0;background:transparent;color:#eef0f5;font-size:16px}.cards{flex:1;overflow-y:auto;padding:0 12px 92px}.cards>button{width:100%;display:grid;grid-template-columns:58px minmax(0,1fr) 24px;align-items:center;gap:12px;padding:11px 8px;border:0;border-top:1px solid #292d35;background:transparent;color:inherit;text-align:left}.cards>button:first-child{border-top:0}.cards>button.active{border-radius:9px;background:#202633}.avatar{width:58px;height:58px;display:grid;place-items:center;overflow:hidden;border:1px solid #3b414c;border-radius:10px;background:#292d36;font-size:20px}.avatar img{width:100%;height:100%;object-fit:cover}.meta{display:grid;gap:6px;min-width:0}.meta b{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:15px}.meta small{color:#858d9c}.empty{display:grid;place-items:center;gap:7px;padding:54px 20px;text-align:center;color:#8d95a3}.empty b{color:#dfe3eb}.cards p{text-align:center;color:#8d95a3}.add{position:fixed;right:18px;bottom:calc(18px + env(safe-area-inset-bottom));z-index:32;display:flex;align-items:center;gap:7px;padding:12px 16px;border:1px solid #596579;border-radius:999px;background:#293344;color:#e9eef8;box-shadow:0 8px 24px #0008}
