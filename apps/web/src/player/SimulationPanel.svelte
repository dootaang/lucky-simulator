<script lang="ts">
  ...
  let {runtime,version,session,portraitFor,assetFor=()=>null,busy=false,pinned=false,pinnable=true,onaction,onchange=()=>{},onpin=()=>{},onclose}:{runtime:ProjectRuntime;version:number;session:PlaySession;portraitFor:(npcId:string,emotion?:string,outfit?:number)=>string|null;assetFor?:(name:string)=>string|null;busy?:boolean;pinned?:boolean;pinnable?:boolean;onaction:SimulationActionHandler;onchange?:()=>void;onpin?:()=>void;onclose:()=>void}=$props();
</script>
{#snippet body()}
  ...
  <main><ScreenRenderer {runtime} {version} {session} {portraitFor} {assetFor} {busy} {onaction} {onchange}/></main>
{/snippet}
