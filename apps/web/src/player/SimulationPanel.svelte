<script lang="ts">
  ...
  let {runtime,version,assetRevision=0,session,portraitFor,assetFor=()=>null,busy=false,pinned=false,pinnable=true,decisionCards=[],onaction,onchange=()=>{},onpin=()=>{},onclose}:{runtime:ProjectRuntime;version:number;assetRevision?:number;session:PlaySession;portraitFor:(npcId:string,emotion?:string,outfit?:number)=>string|null;assetFor?:(name:string)=>string|null;busy?:boolean;pinned?:boolean;pinnable?:boolean;decisionCards?:DecisionCardModel[];onaction:SimulationActionHandler;onchange?:()=>void;onpin?:()=>void;onclose:()=>void}=$props();
</script>
{#snippet body()}
  ...
  <main><ScreenRenderer {runtime} {version} {assetRevision} {session} {portraitFor} {assetFor} {busy} {onaction} {onchange}/></main>{#if decisionCards.length}<footer class="decision-mirror"><DecisionDock cards={decisionCards} {busy} {onaction}/></footer>{/if}
{/snippet}
