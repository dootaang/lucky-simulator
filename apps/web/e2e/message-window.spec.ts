...
test('긴 채팅은 보이는 대화만 그리며 과거를 읽을 때 새 메시지가 위치를 빼앗지 않는다', async ({ page }) => {
  ...
  const result = await page.evaluate(async (repo) => {
    const [{ mount }, { default: MessageList }, { PlaySession }, { ProjectRuntime }, { defaultCardPreset }] = await Promise.all([
      ...
      import(`/@fs/${repo}/packages/session/src/index.ts`),
      import(`/@fs/${repo}/packages/runtime/src/index.ts`),
    ...
    const hasLatestButton=host.querySelector('.latest-button')!==null;
    return { total: session.messageCount, initialDomCount, finalDomCount, initialHasLatest, topHasFirst, topBeforeAppend, topAfterAppend,hasLatestButton };
  }, repo);
  ...
  expect(result.hasLatestButton).toBe(false);
});
