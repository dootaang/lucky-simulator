...
test('모바일 앱바 → 드로어에서 카드·채팅에 도달한다', async ({ page }) => {
  ...
  await expect(page.getByRole('complementary', { name: /카드와 채팅/ })).toBeVisible();
  await expect(page.getByRole('button', { name: '카드 가져오기', exact: true })).toBeVisible(); // 드로어 안의 것
  // 채팅 탭으로 전환 — 카드가 없으면 안내 문구가 뜬다.
  await page.getByRole('button', { name: '채팅', exact: true }).click();
  await expect(page.getByText(/카드를 선택하면 채팅 목록/)).toBeVisible();
