...
const modulePath = (path: string) => `/@fs/${repo}/${path}`;
const SANITIZER = modulePath('packages/ui/src/sanitize-html.ts');
...
test('카드 HTML이 이스케이프되지 않고 실제 이미지로 렌더된다', async ({ page }) => {
  const html = await page.evaluate(async ([markdownPath, sanitizerPath]) => {
    const markdown = await import(/* @vite-ignore */ markdownPath as string) as { renderMarkdownWithHtml(v: string): string };
    const sanitizer = await import(/* @vite-ignore */ sanitizerPath as string) as { sanitizeHtml(v: string): string };
  ...
  }, [modulePath('packages/ui/src/markdown.ts'), SANITIZER]);
...
test('모델이 매크로 없이 쓴 에셋 이름 이미지도 렌더된다', async ({ page }) => {
  const result = await page.evaluate(async ([risuPath, markdownPath, sanitizerPath]) => {
    const risu = await import(/* @vite-ignore */ risuPath as string) as {
  ...
  }, [modulePath('packages/risu/src/asset-macros.ts'), modulePath('packages/ui/src/markdown.ts'), SANITIZER]);
...
test('감정 명령이 현재 의상 AVIF로 해석된 뒤에도 이미지로 남는다',async({page})=>{
  const result=await page.evaluate(async([risuPath,markdownPath,sanitizerPath])=>{
    const risu=await import(/* @vite-ignore */ risuPath as string)as{resolveAssetMacros(v:string,a:readonly{name:string;type:string;mime:string;bytes:Uint8Array|null}[],o:{outfits:Record<string,number>}):{content:string}};
    const markdown=await import(/* @vite-ignore */ markdownPath as string)as{renderMarkdownWithHtml(v:string):string};
    const sanitizer=await import(/* @vite-ignore */ sanitizerPath as string)as{sanitizeHtml(v:string):string};
