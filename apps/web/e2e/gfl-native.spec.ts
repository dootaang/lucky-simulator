...
test('소녀전선 감정 스프라이트와 Dawn BGM을 태그 노출 없이 실제 화면 요소로 바꾼다',async({page})=>{
  await page.setViewportSize({width:390,height:844});
  ...
  await page.route('http://127.0.0.1:4173/test-llm',route=>route.fulfill({contentType:'application/json',body:JSON.stringify({choices:[{message:{content:'FAMAS가 자신의 장비를 토닥였다.\n[|<img="famas_normal">|"제 정식 명칭은 FAMAS입니다."|]\n[|<img="famas_smile">|"더 궁금한 점이 있으신가요?"|]\n|BGM_Dawn|'}}]})}));
  const simulation=await importGfl(page);
