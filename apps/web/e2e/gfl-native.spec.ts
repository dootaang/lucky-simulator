...
test('소녀전선 각도괄호 태그를 채팅 본문의 감정 스프라이트로 렌더링한다',async({page})=>{
  ...
  const dialogueCards=page.locator('.gfl-say');
  await expect(dialogueCards).toHaveCount(2);
  const mobileDialogue=dialogueCards.first(),body=mobileDialogue.locator('.gfl-say-body');
  await expect(body.getByText('제 정식 명칭은 FAMAS입니다.')).toBeVisible();
  const dialogueLayout=await mobileDialogue.evaluate(element=>{const image=element.querySelector<HTMLElement>('.gfl-say-portrait'),panel=element.querySelector<HTMLElement>('.gfl-say-body'),style=panel?getComputedStyle(panel):null,imageBox=image?.getBoundingClientRect(),panelBox=panel?.getBoundingClientRect();return{display:getComputedStyle(element).display,panelBackground:style?.backgroundImage??'none',panelBorder:style?.borderLeftWidth??'0px',stacked:!!imageBox&&!!panelBox&&panelBox.top>=imageBox.top&&panelBox.top<imageBox.bottom+2};});
  expect(dialogueLayout.display).toBe('block');
  expect(dialogueLayout.panelBackground).not.toBe('none');
  expect(parseFloat(dialogueLayout.panelBorder)).toBeGreaterThan(0);
  expect(dialogueLayout.stacked).toBe(true);
