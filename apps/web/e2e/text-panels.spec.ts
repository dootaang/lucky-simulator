...
test('상태 체인은 채팅에서 제거되고 위젯·영수증에 반영되며 다음 응답으로 갱신된다',async({page})=>{
  ...
  await page.getByRole('button',{name:'시뮬레이션 열기'}).click();dialog=page.getByRole('dialog',{name:'시뮬레이션'});await expect(dialog).toContainText('휴식');await expect(dialog).toContainText('귀환 절차 시작');await expect(dialog).not.toContainText('경계');
});
