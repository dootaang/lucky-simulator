# 표준 게임 스키마 v0 — 확정판 (용사여관 수동 스키마화 실험)

> 2026-07-08 지휘자 작성, 같은 날 사용자 검수 통과 (§5 전부 추천안 채택). M1 관문 실험: "용사여관 룰북을 손으로 스키마화할 수 있는가?" — **결론: 가능. 카드의 시스템 전체가 6개 반복 패턴으로 환원된다.**
> 실물 인스턴스 = [schema/yongsa-inn.v0.json](schema/yongsa-inn.v0.json) (최소 코어, 수작업). 엔진 구현 지시서 = [SPEC-M1-engine-v0.md](SPEC-M1-engine-v0.md).
...
## 5. 확정 결정사항 (2026-07-08 사용자 승인)

1. **크기 분류 방식**: LLM은 크기(S/M/L/XL)만 분류하고 엔진이 스텝 값을 수치화. 단 평판처럼 원본이 축·카테고리별 범위(+3~5 등)를 주는 경우, 카테고리를 LLM이 고르고 엔진이 시드 RNG로 범위 내 값을 결정.
2. **성인 요소 스케일**: 스키마는 중립(scales는 일반 구조), 카드 인스턴스가 포함 여부를 결정. 용사여관 v0 인스턴스는 최소 코어라 affinity만 탑재.
3. **v0 범위**: 최소 코어 = gold·식자재/주류·정산·평판 6축·affinity·방배정·하루 사이클(dayEnd) + 플레이어 EXP 골격. 던전·포로·대장간·음란도/복종도는 M3 모듈로 이월.
4. **랜덤성**: 시드 RNG(결정론). 같은 시드 + 같은 이벤트열 = 같은 상태. 골든 테스트의 전제.
5. **엔티티 수작업**: M2 임포트 컴파일러 전까지 용사여관 1장 한정으로 인스턴스 JSON을 손으로 유지한다.

## 6. 다음 단계

1. ~~§5 답 반영 → SCHEMA-v0 확정판~~ ✅
2. ~~용사여관 최소 코어 인스턴스 JSON 수작업 완성~~ ✅ → [schema/yongsa-inn.v0.json](schema/yongsa-inn.v0.json)
3. ~~엔진 v0 스펙 작성~~ ✅ → [SPEC-M1-engine-v0.md](SPEC-M1-engine-v0.md) (Codex 구현 대기)
4. M0 앱에 "엔진" 탭 추가는 엔진 코어 검증 후. BYOK 채팅 연동은 그다음.
